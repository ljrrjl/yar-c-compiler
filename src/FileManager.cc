...
FileID FileManager::register_file(const std::string& filename, const FileInfo& fileinfo)
{
	...
	if (have_file(file_id))
		return file_id;
	if (fileinfo.get_type() == FileInfo::Type::HEADER)
	{
		auto file = std::make_shared<File>(filename, fileinfo);
		auto header_file = std::make_shared<HeaderFileDecorator>(file);
		_files.insert(std::make_pair(file_id, header_file));
	}
	else if (fileinfo.get_type() == FileInfo::Type::SOURCE)
	{
		auto file = std::make_shared<File>(filename, fileinfo);
		auto source_file = std::make_shared<SourceFileDecorator>(file);
